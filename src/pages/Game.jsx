...
function getDyingNightEligibleGearTargets(gs) {
  return (gs.opponent?.field || [])
    .flatMap(unit =>
      (unit.gear || [])
        .map((gear, gearIndex) => ({
          unitUid: unit.uid,
        ...
        .filter(target => target.cost <= 2)
    );
}

function attackerHasDyingNightAttackEffect(attacker) {
  return (attacker?.gear || []).some(gear => {
    const effect = gear.effectData || gear.effect;
    const actions = Array.isArray(effect?.action) ? effect.action : [];
    return gear.id === "g6" || gear.name === "Dying Night" || actions.includes("CHOOSE_RIVAL_GEAR_MAX_COST_2");
  });
}
...
`;
  ...
  const pendingDyingNightGigAttack = gs.pendingGearEffect?.source === "gigAttack";
  const {
    waitingForOpponent,
  ...
  useSinglePlayerSetup({ isMultiplayer, gs, cardMap, setGs });
...
useEffect(() => {
  if (gs.pendingLegendPeek?.owner === "opponent") {
    setGs(prev => ({ ...prev, pendingLegendPeek: false }));
  }
...
const handleMulligan = useCallback((doIt) => {
  ...
  if (!die) return;
  const finalRoll = result ?? Math.floor(Math.random() * die.sides) + 1;
  const newGs = pickGigDie(gs, index, finalRoll, player === "opponent" ? "opponent" : "player");
  setGs(newGs);
  ...
  const handleCardClick = useCallback((index) => {
    if (gs.phase === PHASES.PLAY) {
      ...
      setGs(prev => {
        if (!prev.pendingGearEffect?.source === "gigAttack") return prev;
  ...
  if (card.type === 'gear') {
    ...
    setGearTarget(index);
    setPendingProgram({ card, cardIndex: index, targetType: card.id === "g6" ? "friendlyGearUnitOrLegend" : "friendlyGearUnit" });
  ...
  if (card.type === "program" && card.id === "p1") {
    if (pendingProgram?.targetType === "friendlyUnit") { setPendingProgram(null); return; }
    setPendingProgram({ card, cardIndex: index, targetType: "friendlyUnit" });
    return;
  }
   ...
   if (card.type === 'program' && card.id === 'p2') {
    ...
    setPendingProgram({ card, cardIndex: index, targetType: "spentUnitMax4" });
    return;
  }

  if (card.type === 'program' && card.id === 'p3') { if (gs.player.gigDice.length === 0) return; setPendingProgram({ card, cardIndex: index, effect: 'p3' }); return; }
  if (card.type === 'program' && card.id === 'p4') { setPendingProgram({ card, cardIndex: index, effect: 'p4' }); return; }
  if (card.type === 'program' && card.id === 'p5') { setPendingProgram({ card, cardIndex: index, targetType: 'unit' }); return; }

  if (card.type === 'program' && card.id === 'p7') {
    const targets = gs.opponent.field.filter(u => !u.spent && (u.cost || 0) <= 3);
    if (targets.length === 0) return;
    setPendingProgram({ card, cardIndex: index, targetType: "enemyUnit" });
    return;
  }
  ...
  if (gearTarget !== null && pendingProgram?.targetType === "friendlyGearUnitOrLegend") {
    ...
    setGs(newGs);
    setGearTarget(null); setPendingProgram(null); setactualIndex(null); setSelectedAttacker(null);
    if (isMultiplayer) mpSave(newGs);
    return;
  }
  if (selectedLegend.faceUp && selectedLegend.keywords?.includes("goSolo")) {
  ...
  const resolveFloorItTarget = useCallback((ownerKey, unit) => {
    ...
    if (program) { newGs.player.hand.splice(pendingProgram.cardIndex, 1); newGs.player.trash.push(program); }
    newGs.message = `Floor It returned ${returnedUnit.name} to hand.`;
    setGs(newGs); setPendingProgram(null); setactualIndex(null); setSelectedAttacker(null);
    if (isMultiplayer) mpSave(newGs);
    return true;
  ...
  if (gearTarget !== null && gs.phase === PHASES.PLAY) {
    ...
    setGs(newGs); setGearTarget(null); setPendingProgram(null); setactualIndex(null); setSelectedAttacker(null);
    if (isMultiplayer) mpSave(newGs);
    return;
  }
  if (pendingProgram?.targetType === "friendlyUnit") {
    ...
    if (card?.effectData) resolveEffect(card.effectData, { state: newGs, player: "player", targetUid: unit.uid });
  ...
  if (gs.phase === PHASES.ATTACK) {
    if (!unit || unit.spent || unit.cantAttack || (unit.justPlayed && !unit.canAttackSpentUnitsThisTurn)) return;
  ...
  setSelectedAttacker(null); setactualIndex(null);
  ...
  if (gs.pendingGearEffect?.source === "gigAttack") {
    const targets = (gs.pendingGearEffect.targets || []).filter(target => target.unitUid === unit.uid);
    if (targets.length > 0) {
      setGs(prev => ({
        ...prev,
        pendingGearEffect: {
          ...prev.pendingGearEffect,
          targets,
          choosingGear: true,
          title: "CHOOSE GEAR TO DESTROY",
          description: `Choose Gear attached to ${unit.name}.`,
        }
      }));
      return;
    }
  ...
  if (pendingProgram) {
    ...
    if (card?.id === "p7" && !unit.spent && (unit.cost || 0) <= 3) {
      const newGs = resolvePendingEffect(gs, unit.uid);
      setGs(newGs); setPendingProgram(null); setSelectedAttacker(null);
      if (isMultiplayer) mpSave(newGs);
      return;
    }
  }
  ...
  if (gs.phase === PHASES.ATTACK && selectedAttacker && unit.spent) {
    ...
    setGs(newGs); setSelectedAttacker(null);
    if (isMultiplayer) mpSave(newGs);
  } else {
    setDetailCard(unit);
  }
}, [gs, pendingProgram, selectedAttacker, isMultiplayer, mpSave, resolveFloorItTarget]);
...
const handleAttackGig = useCallback((gigIndex) => {
  ...
  const attacker = gs.player.field.find(u => u.uid === selectedAttacker);
  const eligibleTargets = getDyingNightEligibleGearTargets(gs);

  if (
    attackerHasDyingNightAttackEffect(attacker) &&
    (gs.player.streetCred || 0) >= 7 &&
    ...
    setGs(prev => ({
      ...
      pendingGearEffect: {
        ...
        title: "CHOOSE UNIT",
        description: "Choose an enemy Unit with eligible Gear. The Gig steal resolves after this effect.",
      }
    }));
    return;
  }
  ...
  const handleStartAttack = useCallback(() => {
    ...
    setGs(newGs); setactualIndex(null);
    if (isMultiplayer) mpSave(newGs);
  }, [gs, isMultiplayer, mpSave]);

  const handleEndTurn = useCallback(() => {
    setactualIndex(null); setSelectedAttacker(null); setGearTarget(null); setPendingProgram(null); setRolledThisTurn(false);
  ...
  const handleBlockerDecision = useCallback((blockerUid) => { const newGs = resolveBlockerDecision(gs, blockerUid); setGs(newGs); if (isMultiplayer) mpSave(newGs); }, [gs, isMultiplayer, mpSave]);
  ...
  const handleAdjustGig = useCallback((gigIndex, adjustment) => {
    setGs(prev => {
      ...
      if (program) { updated.player.hand = updated.player.hand.filter(c => c.id !== 'p4'); updated.player.trash.push(program); }
      if (isMultiplayer) mpSave(updated);
      return updated;
    });
  }, [isMultiplayer, mpSave]);

  const handleLegendPeekClose = useCallback(() => { setPeekedLegend(null); setPeekIndex(null); setGs(prev => ({ ...prev, pendingLegendPeek: false })); }, []);
  ...
  const handleGigBoost = useCallback((gigId) => { const newGs = resolveGigBoost(gs, gigId); setGs(newGs); if (isMultiplayer) mpSave(newGs); }, [gs, isMultiplayer, mpSave]);

  const handleLegendPeek = useCallback((index) => {
  if (peekIndex !== null) return;
  const owner = gs.pendingLegendPeek?.owner || "player";
  if (owner !== "player") return;
  const selectedLegend = owner === "player" ? gs.player?.legends?.[index] : gs.opponent?.legends?.[index];
  if (!selectedLegend) return;
  setPeekIndex(index); setPeekedLegend(selectedLegend);
}, [gs, peekIndex]);

  const handleNewGame = useCallback(() => {
  setactualIndex(null); setSelectedAttacker(null); setGearTarget(null); setPendingProgram(null);
  ...
  const canPlay = actualIndex !== null && gs.player?.hand[actualIndex] && (getAvailableEddies(gs.player) + getAvailableLegendEddies(gs.player)) >= (gs.player.hand[actualIndex]?.cost || 0);

const { phaseButtonLabel, phaseButtonDisabled, phaseButtonStyle } = useGameViewState({ gs, disableActions, isGameOver, gearTarget, selectedAttacker, actualIndex, canPlay, canSell, attackBtn });

return (
<div className="min-h-screen w-screen flex flex-col relative overflow-y-auto scanlines" style={{ background: '#020d18' }}>
  <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: 'linear-gradient(rgba(0,255,255,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(0,255,255,0.04) 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
  <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] pointer-events-none" style={{ background: 'radial-gradient(ellipse, rgba(0,255,255,0.06) 0%, transparent 70%)' }} />
      <GameTopBar showRules={showRules} setShowRules={setShowRules} setShowCombatLog={setShowCombatLog} handleLeaveRoom={handleLeaveRoom} />
      <GameOverOverlay isGameOver={isGameOver} gs={gs} handleNewGame={handleNewGame} navigate={navigate} isMultiplayer={isMultiplayer} setShowCombatLog={setShowCombatLog} />
      <WaitingForOpponentOverlay isMultiplayer={isMultiplayer} waitingForOpponent={waitingForOpponent} isGameOver={isGameOver} myRoleRef={myRoleRef} />
      <MulliganOverlay gs={gs} isMultiplayer={isMultiplayer} handleMulligan={handleMulligan} mulliganPreview={mulliganPreview} setMulliganPreview={setMulliganPreview} mousePos={mousePos} setMousePos={setMousePos} />
  ...
  <div className="w-full max-w-[1200px]">
   <PlayerArea
      player={gs.opponent}
      pendingProgram={pendingDyingNightGigAttack ? { targetType: "gearUnitChoice" } : selectedAttackerOnlySpentUnits ? { targetType: "spentUnitAttack" } : pendingProgram}
      pendingGearEffect={gs.pendingGearEffect}
  ...
  <div className="w-full max-w-[1200px] flex justify-center mt-12">
    <PlayerArea
      ...
      selectedAttacker={selectedAttacker}
      playerLabel={isMultiplayer ? myPlayerLabel : localStorage.getItem("cpTCG_playerName") || "Player 1"}
      pendingBlock={gs.pendingBlock}
      onBlock={handleBlockerDecision}
    />
  </div>
  <div className="w-full max-w-[1200px] mt-2">
  <GameActionBar actionBtn={actionBtn} phaseButtonStyle={phaseButtonStyle} phaseButtonLabel={phaseButtonLabel} phaseButtonDisabled={phaseButtonDisabled} handleStartAttack={handleStartAttack} pendingBlock={gs.pendingBlock} passBtn={passBtn} handleBlockerDecision={handleBlockerDecision} endTurnBtn={endTurnBtn} handleEndTurn={handleEndTurn} />
  <HandArea onPlayCard={handlePlayCard} onSellCard={handleSellCard} hand={gs.player.hand} onCardClick={!disableActions ? handleCardClick : () => {}} actualIndex={actualIndex} phase={gs.phase} availableEddies={getAvailableEddies(gs.player) + getAvailableLegendEddies(gs.player)} />
</div>
 {showCombatLog && (
  <GameLog logs={gs.gameLog} alwaysExpanded cardLookup={cardMap} extraHeaderRight={<div className="flex items-center gap-2"><button onClick={() => { const text = (gs.gameLog || []).map(entry => typeof entry === "string" ? entry : entry.msg || "").join("\n"); navigator.clipboard.writeText(text); }} className="text-xs px-2 py-1 border border-cyan-400 rounded hover:bg-cyan-500/20">Copy</button><button onClick={() => setShowCombatLog(false)} className="text-xs px-2 py-1 border border-red-400 text-red-300 rounded hover:bg-red-500/20">Close</button></div>} />
)}
   </div>
      <GameModals
  gs={gs}
